...
import (
	...
	"github.com/prismachain/go-prisma/internal/utesting"
...
func TestEthSuite(t *testing.T) {
	...
	gprisma, err := runGeth("./testdata", jwtPath)
	if err != nil {
		t.Fatalf("could not run gprisma: %v", err)
	}
	defer gprisma.Close()

	suite, err := NewSuite(gprisma.Server().Self(), "./testdata", gprisma.HTTPAuthEndpoint(), common.Bytes2Hex(secret[:]))
	if err != nil {
...
// runGeth creates and starts a gprisma node
