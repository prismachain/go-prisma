...
Use `devp2p enrdump <base64>` to verify and display an Prisma  Node Record.
...
1. initialize the gprisma node with the `genesis.json` file
2. import blocks from `chain.rlp`
3. run the client using the resulting database. For gprisma, use a command like the one below:
...
[dns-tutorial]: https://gprisma.ethereum.org/docs/developers/gprisma-developer/dns-discovery-setup
