...
import (
	...
	"github.com/prismachain/go-prisma/accounts/keystore"
	"github.com/prismachain/go-prisma/common"
	"github.com/prismachain/go-prisma/common/fdlimit"
	...
	"github.com/prismachain/go-prisma/crypto"
	"github.com/prismachain/go-prisma/crypto/kzg4844"
	"github.com/prismachain/go-prisma/eth"
	"github.com/prismachain/go-prisma/eth/catalyst"
	"github.com/prismachain/go-prisma/eth/downloader"
	"github.com/prismachain/go-prisma/eth/ethconfig"
	"github.com/prismachain/go-prisma/eth/filters"
	"github.com/prismachain/go-prisma/eth/gasprice"
	"github.com/prismachain/go-prisma/eth/tracers"
	"github.com/prismachain/go-prisma/ethdb"
	"github.com/prismachain/go-prisma/ethdb/remotedb"
	"github.com/prismachain/go-prisma/ethstats"
	"github.com/prismachain/go-prisma/graphql"
	"github.com/prismachain/go-prisma/internal/ethapi"
	"github.com/prismachain/go-prisma/internal/flags"
	"github.com/prismachain/go-prisma/log"
	"github.com/prismachain/go-prisma/metrics"
	"github.com/prismachain/go-prisma/metrics/exp"
	"github.com/prismachain/go-prisma/metrics/influxdb"
	...
	"github.com/prismachain/go-prisma/p2p/nat"
	"github.com/prismachain/go-prisma/p2p/netutil"
	"github.com/prismachain/go-prisma/params"
	"github.com/prismachain/go-prisma/rpc"
	"github.com/prismachain/go-prisma/triedb"
	"github.com/prismachain/go-prisma/triedb/hashdb"
	"github.com/prismachain/go-prisma/triedb/pathdb"
...
var (
	...
	HttpHeaderFlag = &cli.StringSliceFlag{
		Name:     "header",
		Aliases:  []string{"H"},
		Usage:    "Pass custom headers to the RPC server when using --" + RemoteDBFlag.Name + " or the gprisma attach console. This flag can be given multiple times.",
		Category: flags.APICategory,
	}
...
// for Gprismaand returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gprisma account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Prisma  client to the stack.
// The second return value is the full node instance.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Prisma ) {
	backend, err := eth.New(stack, cfg)
	if err != nil {
		Fatalf("Failed to register the Prisma  service: %v", err)
	}
	stack.RegisterAPIs(tracers.APIs(backend.APIBackend))
	return backend.APIBackend, backend
}

// RegisterEthStatsService configures the Prisma  Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Prisma  Stats service: %v", err)
	}
}
...
func RegisterFullSyncTester(stack *node.Node, eth *eth.Prisma , target common.Hash) {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gprisma.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gprisma.", tagsMap)
		}
...
func DialRPCWithHeaders(endpoint string, headers []string) (*rpc.Client, error) {
	...
	if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gprisma < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
