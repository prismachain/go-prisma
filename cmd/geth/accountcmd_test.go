...
func TestAccountListEmpty(t *testing.T) {
	t.Parallel()
	gprisma := runGeth(t, "account", "list")
	gprisma.ExpectExit()
}
...
`
	...
	{
		gprisma := runGeth(t, "account", "list", "--datadir", datadir)
		gprisma.Expect(want)
		gprisma.ExpectExit()
	}
	{
		gprisma := runGeth(t, "--datadir", datadir, "account", "list")
		gprisma.Expect(want)
		gprisma.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	t.Parallel()
	gprisma := runGeth(t, "account", "new", "--lightkdf")
	defer gprisma.ExpectExit()
	gprisma.Expect(`
...
func TestAccountHelp(t *testing.T) {
	t.Parallel()
	gprisma := runGeth(t, "account", "-h")
	gprisma.WaitExit()
	if have, want := gprisma.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gprisma = runGeth(t, "account", "import", "-h")
	gprisma.WaitExit()
	if have, want := gprisma.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gprisma := runGeth(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gprisma.ExpectExit()
	gprisma.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	t.Parallel()
	datadir := tmpDatadirWithKeystore(t)
	gprisma := runGeth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	t.Parallel()
	gprisma := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gprisma.ExpectExit()
	gprisma.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gprisma.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	t.Parallel()
	gprisma := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gprisma.StderrText(), m) {
...
// https://github.com/prismachain/go-prisma/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gprisma.SetTemplateFunc("keypath", func(file string) string {
