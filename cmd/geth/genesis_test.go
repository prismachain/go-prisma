...
// Tests that initializing Gprismawith a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	...
	for i, tt := range customGenesisTests {
		...
		gprisma := runGeth(t, "--networkid", "1337", "--syncmode=full", "--cache", "16",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0", "--authrpc.port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		gprisma.ExpectRegexp(tt.result)
		gprisma.ExpectExit()
	}
}
...
func TestCustomBackend(t *testing.T) {
	...
	testfunc := func(t *testing.T, tt backendTest) error {
		...
		{ // Init
			...
			gprisma := runGeth(t, args...)
			gprisma.ExpectRegexp(tt.initExpect)
			gprisma.ExpectExit()
		}
		{ // Exec + query
			...
			gprisma := runGeth(t, args...)
			gprisma.ExpectRegexp(tt.execExpect)
			gprisma.ExpectExit()
		}
		return nil
	}
	for i, tt := range []backendTest{
		...
		{ // Can't start pebble on top of leveldb
			initArgs:   []string{"--db.engine", "leveldb"},
			execArgs:   []string{"--db.engine", "pebble"},
			execExpect: `Fatal: Failed to register the Prisma  service: db.engine choice was pebble but found pre-existing leveldb database in specified data directory`,
		},
		{ // Can't start leveldb on top of pebble
			initArgs:   []string{"--db.engine", "pebble"},
			execArgs:   []string{"--db.engine", "leveldb"},
			execExpect: `Fatal: Failed to register the Prisma  service: db.engine choice was leveldb but found pre-existing pebble database in specified data directory`,
		},
