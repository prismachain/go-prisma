...
import (
	...
	"github.com/prismachain/go-prisma/accounts/external"
	"github.com/prismachain/go-prisma/accounts/keystore"
	"github.com/prismachain/go-prisma/accounts/scwallet"
	"github.com/prismachain/go-prisma/accounts/usbwallet"
...
// makeConfigNode loads gprisma configuration and creates a blank node instance.
...
// makeFullNode loads gprisma configuration and creates the Prisma  backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	// Create gauge with gprisma system and build information
	if eth != nil { // The 'eth' backend may be nil in light mode
		var protos []string
		for _, p := range eth.Protocols() {
			protos = append(protos, fmt.Sprintf("%v/%d", p.Name, p.Version))
		}
		metrics.NewRegisteredGaugeInfo("gprisma/info", nil).Update(metrics.GaugeInfoValue{
			"arch":      runtime.GOARCH,
			"os":        runtime.GOOS,
	...
	// Add the Prisma  Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
