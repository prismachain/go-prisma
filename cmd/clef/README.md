...
This setup allows a DApp to connect to a remote Prisma  node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote Prisma  node, because a local one is not available, not synchronized with the chain, or is a node that has no built-in (or limited) account management.
...
The general flow for signing a transaction using e.g. Gprismais as follows:
![image](sign_flow.png)

In this case, `gprisma` would be started with `--signer http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
* Gprismatodos
    ...
    - Relay: Gprismashould be started in `gprisma --signer localhost:8550`.
    - Currently, the GprismaAPIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This type is 20 `bytes`, and is incapable of carrying checksum information. The signer uses `common.MixedcaseAddress`, which retains the original input.
    - The GprismaAPI should switch to use the same type, and relay `to`-account verbatim to the external API.
* [x] Storage
