...
// Package gethclient provides an RPC client for gprisma-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements gprisma-specific functionality.
//
// If you want to use the standardized Prisma  RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
// GCStats retrieves the current garbage collection stats from a gprisma node.
...
// MemStats retrieves the current memory stats from a gprisma node.
...
// GetNodeInfo retrieves the node info of a gprisma node.
