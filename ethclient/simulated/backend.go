...
// Client exposes the methods provided by the Prisma  RPC client.
...
// other code that interacts with the Prisma  chain.
type Backend struct {
	eth    *eth.Prisma 
...
func NewBackend(alloc types.GenesisAlloc, options ...func(nodeConf *node.Config, ethConf *ethconfig.Config)) *Backend {
	// Create the default configurations for the outer node shell and the Prisma 
	...
	// Assemble the Prisma  stack to run the chain with
