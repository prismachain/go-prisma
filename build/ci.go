...
import (
	...
	"github.com/prismachain/go-prisma/crypto/signify"
	"github.com/prismachain/go-prisma/internal/build"
	"github.com/prismachain/go-prisma/params"
)

var (
	// Files that end up in the gprisma*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gprisma"),
	}

	// Files that end up in the gprisma-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Prisma  contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Prisma  bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Prisma  Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gprisma",
			Description: "Prisma  CLI client.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Prisma  account management tool.",
		},
	}
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	var ld []string
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/prismachain/go-prisma/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/prismachain/go-prisma/internal/version.gitDate="+env.Date)
	}
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gprisma     = "gprisma-" + basegeth + ext
		alltools = "gprisma-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gprisma, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gprisma, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gprisma-release.pub"
...
func doDocker(cmdline []string) {
	...
	//  - ethereum/client-go:latest                            - Pushes to the master branch, Gprismaonly
	//  - ethereum/client-go:stable                            - Version tag publish on GitHub, Gprismaonly
	//  - ethereum/client-go:alltools-latest                   - Pushes to the master branch, Gprisma& tools
	//  - ethereum/client-go:alltools-stable                   - Version tag publish on GitHub, Gprisma& tools
	//  - ethereum/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Gprismaonly
	//  - ethereum/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Gprisma& tools
	//  - ethereum/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Gprismaonly
	//  - ethereum/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Gprisma& tools
	var tags []string
	...
	if len(*manifest) != 0 {
		...
		// Assemble and push the Gprismamanifest image
		for _, tag := range tags {
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gprisma-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gprisma-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Prisma  Builds <fjl@ethereum.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gprisma.exe" {
			gethTool = file
		} else {
	...
	// first section contains the gprisma binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gprisma.nsi", filepath.Join(*workdir, "gprisma.nsi"), 0644, nil)
	...
	installer, err := filepath.Abs("gprisma-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gprisma.nsi"),
	)
