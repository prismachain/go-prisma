...
import (
	...
	"github.com/prismachain/go-prisma/crypto/bls12381"
...
func fuzzCrossPairing(data []byte) int {
	...
	if !(bytes.Equal(cResult.Marshal(), bls12381.NewGT().ToBytes(kResult))) {
		panic("pairing mismatch gnark / gprisma ")
	}
	...
	if !(bytes.Equal(res, bls12381.NewGT().ToBytes(kResult))) {
		panic("pairing mismatch blst / gprisma")
	}
...
func fuzzCrossG1Add(data []byte) int {
	...
	if !(bytes.Equal(cp.Marshal(), g1.ToBytes(&kp))) {
		panic("G1 point addition mismatch gnark / gprisma ")
	}

	bl3 := blst.P1AffinesAdd([]*blst.P1Affine{bl1, bl2})
	if !(bytes.Equal(cp.Marshal(), bl3.Serialize())) {
		panic("G1 point addition mismatch blst / gprisma ")
	}
...
func fuzzCrossG2Add(data []byte) int {
	...
	if !(bytes.Equal(cp.Marshal(), g2.ToBytes(&kp))) {
		panic("G2 point addition mismatch gnark / gprisma ")
	}
...
func fuzzCrossG1MultiExp(data []byte) int {
	...
	for i := 0; i < 17; i++ {
		// note that gprisma/crypto/bls12381 works only with scalars <= 32bytes
	...
	if _, err := g1.MultiExp(&kp, gethPoints, gethScalars); err != nil {
		panic(fmt.Sprintf("G1 multi exponentiation errored (gprisma): %v", err))
	}
	// note that gprisma/crypto/bls12381.MultiExp mutates the scalars slice (and sets all the scalars to zero)
	...
	// compare result
	if !(bytes.Equal(cp.Marshal(), g1.ToBytes(&kp))) {
		panic("G1 multi exponentiation mismatch gnark / gprisma ")
	}
...
func getG1Points(input io.Reader) (*bls12381.PointG1, *gnark.G1Affine, *blst.P1Affine, error) {
	...
	// marshal gnark point -> gprisma point
	g1 := bls12381.NewG1()
	kp, err := g1.FromBytes(cpBytes)
	if err != nil {
		panic(fmt.Sprintf("Could not marshal gnark.G1 -> gprisma.G1: %v", err))
	}
	if !bytes.Equal(g1.ToBytes(kp), cpBytes) {
		panic("bytes(gnark.G1) != bytes(gprisma.G1)")
	}
	...
	if !bytes.Equal(p1.Serialize(), cpBytes) {
		panic("bytes(blst.G1) != bytes(gprisma.G1)")
	}
...
func getG2Points(input io.Reader) (*bls12381.PointG2, *gnark.G2Affine, *blst.P2Affine, error) {
	...
	if err != nil {
		panic(fmt.Sprintf("Could not marshal gnark.G2 -> gprisma.G2: %v", err))
	}
	if !bytes.Equal(g2.ToBytes(kp), cpBytes) {
		panic("bytes(gnark.G2) != bytes(gprisma.G2)")
	}
	...
	if !bytes.Equal(p2.Serialize(), cpBytes) {
		panic("bytes(blst.G2) != bytes(gprisma.G2)")
	}
