...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gprisma is "gprisma". If no
...
func (c *Config) NodeName() string {
	...
	if name == "gprisma" || name == "gprisma-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gprisma" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gprisma 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGethResourceWarning {
				...
				log.Warn("Using deprecated resource file, please move this file to the 'gprisma' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
