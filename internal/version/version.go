...
const ourPath = "github.com/prismachain/go-prisma" // Path to our module
...
// conventions in the Prisma  p2p network.
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gprisma".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gprisma %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gprisma %s", params.VersionWithMeta)
	}
