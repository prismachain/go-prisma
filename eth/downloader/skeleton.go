...
// the process. This is usually the case when Gprismais shutting down and some events
...
//
// Opposed to the original Prisma  block synchronization which is trustless (and
...
// not from within (vs. legacy Prisma  sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gprismais shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gprismarequests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
