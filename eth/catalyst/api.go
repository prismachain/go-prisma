...
import (
	...
	"github.com/prismachain/go-prisma/params/forks"
	"github.com/prismachain/go-prisma/rpc"
)

// Register adds the engine API to the full node.
func Register(stack *node.Node, backend *eth.Prisma ) error {
...
type ConsensusAPI struct {
	...
	//     persist any bad block information to disk as a bug in Gprismacould end
	//     up blocking a valid chain, even if a later Gprismaupdate would accept
	//     it.
	//   - Bad blocks will get forgotten after a certain threshold of import
	//     attempts and will be retried. The rationale is that if the network
	//     really-really-really tries to feed us a block, we should give it a
	//     new chance, perhaps us being racey instead of the block being legit
	//     bad (this happened in Gprismaat a point with import vs. pending race).
	...
	// Gprismacan appear to be stuck or do strange things if the beacon client is
...
func NewConsensusAPI(eth *eth.Prisma ) *ConsensusAPI {
...
func newConsensusAPIWithoutHeartbeat(eth *eth.Prisma ) *ConsensusAPI {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config engine.TransitionConfigurationV1) (*engine.TransitionConfigurationV1, error) {
	...
	if ttd == nil || ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "gprisma", ttd, "beacon", config.TerminalTotalDifficulty)
